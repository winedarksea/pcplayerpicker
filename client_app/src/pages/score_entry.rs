    ...
    F: Fn(MatchResult) + 'static + Clone,
{
    ...
    let team_a_points_draft = RwSignal::new(
        initial_result
            ...
            .and_then(|result| {
                ...
                result.numeric_team_points(&scheduled_match).map(|(team_a_points, _)| team_a_points)
            })
            .unwrap_or(0)
            .to_string(),
    );
    let team_b_points_draft = RwSignal::new(
        initial_result
            ...
            .and_then(|result| {
                ...
                result.numeric_team_points(&scheduled_match).map(|(_, team_b_points)| team_b_points)
            })
            .unwrap_or(0)
            .to_string(),
    );
    let outcome_draft = RwSignal::new(match existing_result.as_ref().map(|result| &result.score_payload)
    {
    ...
    view! {
        <div class="bg-gray-900 border border-gray-700/50 rounded-xl overflow-hidden">
                                    ...
                                    <input
                                        ...
                                        on:input=move |ev| {
                                            player_points_draft.update(|draft| {
                                                draft.insert(player_id, parse_non_negative_u16(&event_target_value(&ev)));
                                            });
                                        }
                                    />
                                })}
            ...
            {(score_entry_mode == ScoreEntryMode::PointsPerTeam).then(|| view! {
                <div class="px-4 pb-3 border-t border-gray-700/20 pt-3 space-y-3">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <input
                            ...
                            on:input=move |ev| team_a_points_draft.set(event_target_value(&ev))
                        />
                        <input
                            type="number"
                            min="0"
                            ...
                            on:input=move |ev| team_b_points_draft.set(event_target_value(&ev))
                        />
                    </div>
                </div>
            })}

            {(score_entry_mode == ScoreEntryMode::WinDrawLose).then(|| view! {
                        ...
                        {[
                            ...
                            view! {
                                <button
                                    ...
                                    on:click=move |_| outcome_draft.set(outcome)
                                >
                                    {label}
                                </button>
                            }
            ...
            {show_duration_picker.then(|| view! {
                <div class="px-4 pb-3 border-t border-gray-700/20 pt-3">
                        ...
                        {DURATION_OPTIONS.iter().map(|&(value, label)| {
                            view! {
                                <button
                                    ...
                                    on:click=move |_| duration_mult.set(value)
                                >
                                    {label}
                                </button>
                            }
